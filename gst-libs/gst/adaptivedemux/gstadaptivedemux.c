...
#define GST_ADAPTIVE_DEMUX_FLOW_UNSELECTED (GST_FLOW_CUSTOM_SUCCESS_2 + 2)
...
struct _GstAdaptiveDemuxOutputSlot
{
  ...
  /* protected by manifest_lock */
  GstAdaptiveDemuxStream *active_stream;
  GstAdaptiveDemuxStream *pending_stream;
  gboolean exposed;
};
...
static void gst_adaptive_demux_slot_free (GstAdaptiveDemuxOutputSlot * stream);
static GstFlowReturn
...
gst_adaptive_demux_reset (GstAdaptiveDemux * demux)
{
  ...
  if (demux->priv->collection) {
    gst_object_unref (demux->priv->collection);
    demux->priv->collection = NULL;
  }
...
static GstAdaptiveDemuxOutputSlot *
gst_adaptive_demux_create_slot (GstAdaptiveDemux * demux, GstStreamType type)
{
  GstAdaptiveDemuxOutputSlot *slot;
  gchar *pad_name;
  const gchar *prefix;
  guint32 *counter;

  slot = g_new0 (GstAdaptiveDemuxOutputSlot, 1);
  ...
  *counter += 1;
  ...
  gst_pad_set_element_private (slot->src_pad, slot);
  gst_pad_set_query_function (slot->src_pad,
      GST_DEBUG_FUNCPTR (gst_adaptive_demux_src_query));
  gst_pad_set_event_function (slot->src_pad,
...
static gboolean
gst_adaptive_demux_slot_link (GstAdaptiveDemuxOutputSlot * slot,
    GstAdaptiveDemuxStream * stream)
{
...
static gboolean
gst_adaptive_demux_slot_expose (GstAdaptiveDemuxOutputSlot * slot)
{
  gboolean ret = TRUE;
  GstAdaptiveDemux *demux = slot->demux;

  if (!slot->pending_stream) {
    GST_ERROR_OBJECT (slot->src_pad, "Slot has no pending stream");
    return FALSE;
  }

  slot->active_stream = slot->pending_stream;
  demux->priv->active_selection =
      g_list_append (demux->priv->active_selection,
      g_strdup (gst_stream_get_stream_id (slot->active_stream->stream_object)));

  /* Don't hold the manifest lock while exposing a pad */
  if (!slot->exposed) {
    GstAdaptiveDemuxStream *stream = slot->active_stream;
    ...
    ret = gst_element_add_pad (GST_ELEMENT_CAST (slot->demux), slot->src_pad);
...
static gboolean
gst_adaptive_demux_slot_unlink (GstAdaptiveDemuxOutputSlot * slot)
{
  gboolean ret = TRUE;

  if (!gst_ghost_pad_set_target ((GstGhostPad *) slot->src_pad, NULL)) {
    GST_ERROR_OBJECT (slot->src_pad, "Could not unset target pad");
    ret = FALSE;
  }

  if (slot->active_stream)
    slot->active_stream->slot = NULL;
...
static void
gst_adaptive_demux_slot_free (GstAdaptiveDemuxOutputSlot * slot)
{
...
static GstAdaptiveDemuxOutputSlot *
gst_adaptive_demux_find_free_compatible_slot (GstAdaptiveDemuxStream * stream)
{
  GList *iter;
  GstAdaptiveDemuxOutputSlot *ret = NULL;
  GstCaps *caps;
  GstStreamType type;
  ...
  for (iter = demux->priv->slots; iter; iter = g_list_next (iter)) {
    GstAdaptiveDemuxOutputSlot *slot =
        (GstAdaptiveDemuxOutputSlot *) iter->data;
    const GstStructure *other_str;

    /* This slot will be used for other stream */
    if (slot->pending_stream)
      continue;
    ...
    other_caps = gst_pad_get_current_caps (slot->src_pad);
    ...
    GstAdaptiveDemuxStream * stream)
{
  ...
  if (demux->streams_aware) {
    ...
    if (!stream_in_list (demux->priv->requested_selection, sid)) {
      GST_DEBUG_OBJECT (stream->pad, "Skip exposing unselected stream %s", sid);
      goto beach;
    }

    slot = gst_adaptive_demux_find_free_compatible_slot (stream);

    if (slot) {
      gst_adaptive_demux_slot_unlink (slot);
      if (!gst_adaptive_demux_slot_link (slot, stream)) {
        ret = FALSE;
        goto beach;
      }
      ...
      if (caps && !gst_caps_is_any (caps)) {
        ...
        ret = gst_adaptive_demux_slot_expose (slot);
      } else {
        GST_DEBUG_OBJECT (stream->pad, "Unknown stream caps, link them later");
      }
    } else {
      ...
      ret = gst_adaptive_demux_slot_link (slot, stream);
    }
...
gst_adaptive_demux_expose_streams (GstAdaptiveDemux * demux)
{
  ...
  /* Clear pending streams to update them in _expose_stream() */
  for (iter = demux->priv->slots; iter; iter = g_list_next (iter)) {
    GstAdaptiveDemuxOutputSlot *slot =
        (GstAdaptiveDemuxOutputSlot *) iter->data;
  ...
  if (old_streams) {
    ...
    for (iter = old_streams; iter; iter = g_list_next (iter)) {
      GstAdaptiveDemuxStream *stream = iter->data;
      GstPad *pad = stream->pad;
      GstAdaptiveDemuxOutputSlot *slot = stream->slot;

      stream->slot = NULL;

      /* Unlink stream from slot, if any */
      if (slot) {
        slot->active_stream = NULL;
...
gst_adaptive_demux_is_selection_done (GstAdaptiveDemux * demux)
{
  ...
  if (g_list_length (demux->priv->requested_selection) !=
      g_list_length (demux->priv->active_selection)) {
...
gst_adaptive_demux_stream_src_probe (GstPad * pad, GstPadProbeInfo * info,
    GstAdaptiveDemuxStream * stream)
{
  GstAdaptiveDemux *demux = stream->demux;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_CAPS:
    {
      ...
      GST_MANIFEST_LOCK (demux);
      if (stream->slot == NULL) {
        GST_LOG_OBJECT (pad, "Stream %p has no slot", stream);
        GST_MANIFEST_UNLOCK (demux);
        break;
      } else if (stream->slot && stream->slot->active_stream == stream) {
        GST_LOG_OBJECT (pad, "Stream %p is linked with slot %p %"
            GST_PTR_FORMAT, stream, stream->slot, stream->slot->src_pad);
        GST_MANIFEST_UNLOCK (demux);
        break;
      }
      ...
      if (slot->exposed) {
        GstCaps *caps, *other_caps;
        const GstStructure *str, *other_str;
        ...
        if (g_strcmp0 (gst_structure_get_name (str),
                gst_structure_get_name (other_str)) != 0) {
          GstAdaptiveDemuxOutputSlot *to_free_slot =
              (GstAdaptiveDemuxOutputSlot *) to_free->data;

          GST_DEBUG_OBJECT (pad, "Cannot reuse slot %p %" GST_PTR_FORMAT
              ", create new one", slot, slot->src_pad);

          to_free = g_list_find (demux->priv->slots, slot);
          gst_adaptive_demux_slot_unlink (to_free_slot);

          slot = gst_adaptive_demux_create_slot (demux,
              gst_stream_get_stream_type (stream->stream_object));

          /* Preserve linked list order */
          demux->priv->slots =
              g_list_insert_before (demux->priv->slots, to_free, slot);
          demux->priv->slots = g_list_remove_link (demux->priv->slots, to_free);

          gst_adaptive_demux_slot_link (slot, stream);
        } else {
          GST_DEBUG_OBJECT (pad, "Reuse slot %p %" GST_PTR_FORMAT,
              slot, slot->src_pad);
        }
      ...
      GST_DEBUG_OBJECT (slot->src_pad, "Expose slot");
      gst_adaptive_demux_slot_expose (slot);
      msg = gst_adaptive_demux_is_selection_done (demux);
      GST_MANIFEST_UNLOCK (demux);
      ...
      if (to_free) {
        ...
        gst_pad_push_event (to_free_slot->src_pad, gst_event_new_eos ());
        GST_MANIFEST_LOCK (demux);
        g_list_free_full (to_free,
            (GDestroyNotify) gst_adaptive_demux_slot_free);
        GST_MANIFEST_UNLOCK (demux);
      }
    ...
    case GST_EVENT_EOS:{
      ...
      slot = stream->slot;
      sid = gst_stream_get_stream_id (stream->stream_object);

      if (!stream_in_list (demux->priv->requested_selection, sid)) {
        GST_DEBUG_OBJECT (stream->pad, "Remove slot for unselected stream");
        gst_pad_push_event (slot->src_pad, gst_event_new_eos ());
        GST_MANIFEST_LOCK (demux);
        stream->slot = NULL;
        gst_adaptive_demux_slot_unlink (slot);
        demux->priv->slots = g_list_remove (demux->priv->slots, slot);
...
gst_adaptive_demux_stream_new (GstAdaptiveDemux * demux, GstPad * pad)
{
  ...
  if (demux->streams_aware)
    gst_pad_add_probe (pad, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
        (GstPadProbeCallback) gst_adaptive_demux_stream_src_probe, stream,
        NULL);
...
{
  GstAdaptiveDemux *demux;
  GstAdaptiveDemuxOutputSlot *slot = gst_pad_get_element_private (pad);
    ...
    GstEvent * event)
{
  ...
  for (iter = select_streams; iter; iter = g_list_next (iter)) {
    ...
    if (stream == NULL) {
      if (stream_in_collection (demux, (gchar *) sid)) {
        ...
        GST_DEBUG_OBJECT (demux, "We don't have a stream for '%s'", sid);
        unknown = g_list_append (unknown, (gchar *) sid);
      }
    } else if (stream->slot == NULL) {
      GST_DEBUG_OBJECT (demux, "We need to activate slot for stream '%s'", sid);
  ...
  for (iter = to_activate; iter; iter = g_list_next (iter)) {
    GstAdaptiveDemuxStream *stream = (GstAdaptiveDemuxStream *) iter->data;
    GstAdaptiveDemuxOutputSlot *slot = gst_adaptive_demux_create_slot (demux,
        gst_stream_get_stream_type (stream->stream_object));
  ...
  for (iter = demux->streams; iter; iter = g_list_next (iter)) {
    ...
    if (stream->slot == NULL) {
      GST_DEBUG_OBJECT (stream->pad,
          "Skip starting task for unselected stream");
      continue;
    }
...
_src_chain (GstPad * pad, GstObject * parent, GstBuffer * buffer)
{
  ...
  if (ret != GST_FLOW_OK) {
    ...
    if (ret == (GstFlowReturn) GST_ADAPTIVE_DEMUX_FLOW_SWITCH ||
        ret == (GstFlowReturn) GST_ADAPTIVE_DEMUX_FLOW_UNSELECTED) {
  ...
  switch (ret) {
    ...
    default:
      if (ret <= GST_FLOW_ERROR) {
        ...
        goto end;
      } else if (ret == (GstFlowReturn) GST_ADAPTIVE_DEMUX_FLOW_UNSELECTED) {
        GST_DEBUG_OBJECT (demux, "Stop taks for unselected stream");
        gst_task_stop (stream->download_task);
        stream->last_ret = GST_FLOW_NOT_LINKED;
        ret = gst_adaptive_demux_combine_flows (demux);
        GST_MANIFEST_UNLOCK (demux);
    ...
    GstAdaptiveDemuxStream * stream, GstClockTime duration)
{
  ...
  if (ret == GST_FLOW_OK) {
    ...
    } else if (G_UNLIKELY (demux->priv->selection_updated)) {
      const gchar *sid = gst_stream_get_stream_id (stream->stream_object);

      if (!stream_in_list (demux->priv->requested_selection, sid)) {
        GST_DEBUG_OBJECT (stream->pad,
            "Stop task for unselected stream %s", sid);
        ret = (GstFlowReturn) GST_ADAPTIVE_DEMUX_FLOW_UNSELECTED;
      }
    }
  }
