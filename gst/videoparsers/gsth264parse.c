...
static gboolean gst_h264_parse_sink_event (GstBaseParse * parse,
    GstEvent * event);

/* h26xbaseparse */
static void gst_h264_parse_reset (GstH26xBaseParse * parse);
static void gst_h264_parse_reset_frame (GstH26xBaseParse * parse);
...
static GstH26xHandleFrameReturn
gst_h264_parse_handle_frame_check_initial_skip (GstH26xBaseParse * parse,
    gint * skipsize, gint * framesize, GstMapInfo * map);
static GstH26xHandleFrameReturn
gst_h264_parse_handle_frame_bytestream (GstH26xBaseParse * parse,
    gint * skipsize, gint * framesize, gint * current_off, GstMapInfo * map,
    gboolean drain);
...
static void
gst_h264_parse_get_timestamp (GstH26xBaseParse * parse, GstClockTime * ts,
    GstClockTime * dur, gboolean frame);

static gboolean
gst_h264_parse_fixate_format (GstH26xBaseParse * parse, guint * format,
    GstH26xAlign * align, const GValue * codec_data_value);

/* get info frm sps */
static gboolean gsh_h264_parse_has_last_sps (GstH26xBaseParse * parse);
static gboolean
gsh_h264_parse_fill_sps_info (GstH26xBaseParse * parse, GstH26xSPSInfo * info);
static gboolean
...
gst_h264_parse_class_init (GstH264ParseClass * klass)
{
  ...
  GstH26xBaseParseClass *h26xbaseparse_class =
      GST_H26X_BASE_PARSE_CLASS (klass);
  ...
  parse_class->sink_event = GST_DEBUG_FUNCPTR (gst_h264_parse_sink_event);

  h26xbaseparse_class->reset = GST_DEBUG_FUNCPTR (gst_h264_parse_reset);
  h26xbaseparse_class->reset_frame =
      GST_DEBUG_FUNCPTR (gst_h264_parse_reset_frame);
  h26xbaseparse_class->get_max_vps_sps_pps_count =
      GST_DEBUG_FUNCPTR (gst_h264_parse_get_max_vps_sps_pps_count);
  h26xbaseparse_class->format_to_string =
      GST_DEBUG_FUNCPTR (gst_h264_parse_format_to_string);
  h26xbaseparse_class->format_from_string =
      GST_DEBUG_FUNCPTR (gst_h264_parse_format_from_string);
  h26xbaseparse_class->prepare_pre_push_frame =
      GST_DEBUG_FUNCPTR (gst_h264_parse_prepare_pre_push_frame);
  h26xbaseparse_class->make_codec_data =
      GST_DEBUG_FUNCPTR (gst_h264_parse_make_codec_data);
  h26xbaseparse_class->get_default_caps =
      GST_DEBUG_FUNCPTR (gst_h264_parse_get_default_caps);
  h26xbaseparse_class->handle_codec_data =
      GST_DEBUG_FUNCPTR (gst_h264_parse_handle_codec_data);
  h26xbaseparse_class->fixate_format =
      GST_DEBUG_FUNCPTR (gst_h264_parse_fixate_format);

  h26xbaseparse_class->handle_frame_packetized =
      GST_DEBUG_FUNCPTR (gst_h264_parse_handle_frame_packetized);
  h26xbaseparse_class->handle_frame_check_initial_skip =
      GST_DEBUG_FUNCPTR (gst_h264_parse_handle_frame_check_initial_skip);
  h26xbaseparse_class->handle_frame_bytestream =
      GST_DEBUG_FUNCPTR (gst_h264_parse_handle_frame_bytestream);

  h26xbaseparse_class->has_last_sps =
      GST_DEBUG_FUNCPTR (gsh_h264_parse_has_last_sps);
  h26xbaseparse_class->fill_sps_info =
      GST_DEBUG_FUNCPTR (gsh_h264_parse_fill_sps_info);
  h26xbaseparse_class->fill_profile_tier_level =
      GST_DEBUG_FUNCPTR (gst_h264_parse_fill_profile_tier_level);
  h26xbaseparse_class->get_compatible_profile_caps_from_last_sps =
      GST_DEBUG_FUNCPTR
      (gst_h264_parse_get_compatible_profile_caps_from_last_sps);
  h26xbaseparse_class->get_timestamp =
...
static void
gst_h264_parse_reset_frame (GstH26xBaseParse * parse)
{
...
static void
gst_h264_parse_reset (GstH26xBaseParse * parse)
{
  GstH264Parse *h264parse = GST_H264_PARSE (parse);
...
gst_h264_parse_format_from_string (GstH26xBaseParse * parse,
    const gchar * format)
{
  g_return_val_if_fail (format != NULL, GST_H264_PARSE_FORMAT_NONE);

  if (strcmp (format, "avc") == 0)
    return GST_H264_PARSE_FORMAT_AVC;
  else if (strcmp (format, "byte-stream") == 0)
    return GST_H264_PARSE_FORMAT_BYTE;
  else if (strcmp (format, "avc3") == 0)
    return GST_H264_PARSE_FORMAT_AVC3;

  return GST_H264_PARSE_FORMAT_NONE;
}
...
gst_h264_parse_process_nal (GstH264Parse * h264parse, GstH264NalUnit * nalu)
{
  ...
  switch (nal_type) {
    ...
    process_sps:
      ...
      parse->update_caps = TRUE;
      parse->have_sps = TRUE;
      if (parse->push_codec && parse->have_pps) {
        ...
        parse->push_codec = FALSE;
        parse->have_sps = FALSE;
        parse->have_pps = FALSE;
      }
    ...
    case GST_H264_NAL_PPS:
      ...
      parse->have_pps = TRUE;
      if (parse->push_codec && parse->have_sps) {
        /* SPS and PPS found in stream before the first pre_push_frame, no need
         * to forcibly push at start */
        GST_INFO_OBJECT (h264parse, "have SPS/PPS in stream");
        parse->push_codec = FALSE;
      ...
      parse->state |= GST_H26X_BASE_PARSE_STATE_GOT_PPS;
      parse->header |= TRUE;
      break;
    case GST_H264_NAL_SEI:
      ...
      if (parse->sei_pos == -1) {
        if (parse->transform)
          parse->sei_pos = gst_adapter_available (parse->frame_out);
        else
          parse->sei_pos = nalu->sc_offset;
    ...
    case GST_H264_NAL_SLICE_EXT:
      ...
      if (*(nalu->data + nalu->offset + nalu->header_bytes) & 0x80) {
        ...
        parse->frame_start = TRUE;
      }
      GST_DEBUG_OBJECT (h264parse, "frame start: %i", parse->frame_start);
        ...
        break;
      {
        ...
        if (pres == GST_H264_PARSER_OK) {
          ...
          parse->state |= GST_H26X_BASE_PARSE_STATE_GOT_SLICE;
      ...
      if (parse->idr_pos == -1) {
        if (parse->transform)
          parse->idr_pos = gst_adapter_available (parse->frame_out);
        else
          parse->idr_pos = nalu->sc_offset;
        GST_DEBUG_OBJECT (h264parse, "marking IDR in frame at offset %d",
            parse->idr_pos);
      }
      /* if SEI preceeds (faked) IDR, then we have to insert config there */
      if (parse->sei_pos >= 0 && parse->idr_pos > parse->sei_pos) {
        parse->idr_pos = parse->sei_pos;
  ...
  if (parse->transform) {
    ...
    buf = gst_h26x_base_parse_wrap_nal (parse,
        parse->format, nalu->data + nalu->offset, nalu->size);
    gst_adapter_push (parse->frame_out, buf);
  }
  return TRUE;
}
    ...
    GstBaseParseFrame * frame)
{
  ...
  while (parse_res == GST_H264_PARSER_OK) {
    ...
    if (parse->split_packetized) {
      ...
      ret =
          gst_base_parse_finish_frame (GST_BASE_PARSE (parse), &tmp_frame,
          nl + nalu.size);
      left -= nl + nalu.size;
    }
  ...
  if (!parse->split_packetized) {
    ...
    ret = gst_base_parse_finish_frame (GST_BASE_PARSE (parse), frame, map.size);
  } else {
    ...
    if (G_UNLIKELY (left)) {
      ...
      ret =
          gst_base_parse_finish_frame (GST_BASE_PARSE (parse), frame, map.size);
    }
  }
    ...
    gint * skipsize, gint * framesize, GstMapInfo * map)
{
  ...
  switch (pres) {
    case GST_H264_PARSER_OK:
      if (nalu.sc_offset > 0) {
        ...
        if (is_filler_data) {
          ...
          return GST_H26X_HANDLE_FRAME_DROP;
        }
        *skipsize = nalu.sc_offset;
        return GST_H26X_HANDLE_FRAME_SKIP;
      }
      break;
    ...
    default:
      ...
      return GST_H26X_HANDLE_FRAME_INVALID_STREAM;
  }

  return GST_H26X_HANDLE_FRAME_OK;
}
  ...
  while (TRUE) {
    ...
    switch (pres) {
      ...
      case GST_H264_PARSER_NO_NAL_END:
        ...
        /* otherwise need more */
        return GST_H26X_HANDLE_FRAME_MORE;
     ...
     */
    if (parse->align == GST_H26X_BASE_PARSE_ALIGN_NAL && !h264parse->aud_needed)
      h264parse->aud_insert = FALSE;
    ...
    (GstH26xBaseParse * parse)
{
  ...
  if (h264parse->nalparser->last_sps == NULL)
    return NULL;
    ...
    GstBaseParseFrame * frame)
{
   ...
   * if it doesn't exist */
  if (h264parse->aud_insert && parse->format == GST_H264_PARSE_FORMAT_BYTE) {
...
static gboolean
gsh_h264_parse_has_last_sps (GstH26xBaseParse * parse)
{
  GstH264Parse *h264parse = GST_H264_PARSE (parse);
    ...
    GstH26xProfileTierLevel * str)
{
  ...
  str->profile = get_profile_string (sps);
  str->level = get_level_string (sps);
...
gst_h264_parse_handle_codec_data (GstH26xBaseParse * parse, GstMapInfo * map)
{
  ...
  GST_DEBUG_OBJECT (h264parse, "nal length size %u", parse->nal_length_size);
...
static gboolean
gst_h264_parse_sink_event (GstBaseParse * parse, GstEvent * event)
{
  gboolean res;
  GstH264Parse *h264parse = GST_H264_PARSE (parse);
