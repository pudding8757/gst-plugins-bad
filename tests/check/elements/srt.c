...
GST_START_TEST (test_cancel)
{
  GstElement *serversrc = NULL;
  GstElement *clientsink = NULL;
  GstPad *sinkpad = NULL;
  GstPad *srcpad = NULL;
  GstBufferList *buf_list;
  GstSegment segment;

  fail_unless (srtsrc_setup (&serversrc, &sinkpad, "srt://:9999", 2147483647,
          TRUE));
  /* Wait src setup done */
  sleep (1);

  fail_unless (srtsink_setup (&clientsink, &srcpad, "srt://127.0.0.1:9999",
          2147483647, FALSE));
  ...
  gst_check_teardown_pad_by_name (serversrc, "src");
  gst_check_teardown_element (serversrc);

  gst_check_teardown_pad_by_name (clientsink, "sink");
  gst_check_teardown_element (clientsink);
}
...
srt_suite (void)
{
  ...
  tcase_add_test (tc_chain, test_cancel);
