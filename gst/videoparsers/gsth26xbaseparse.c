...
#define IS_PACKETIZED_FORMAT(format) \
  (((format) > GST_H26X_BASE_PARSE_FORMAT_BYTE) ? TRUE : FALSE)
...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE (GstH26xBaseParse, gst_h26x_base_parse,
    GST_TYPE_BASE_PARSE,
    GST_DEBUG_CATEGORY_INIT (h26x_base_parse_debug, "h26xbaseparse", 0,
        "h26x base parser"));
...
gst_h26x_base_parse_init (GstH26xBaseParse * self)
{
  ...
  self->min_nal_size = klass->get_min_nal_size (self);
...
gst_h26x_base_parse_finalize (GObject * object)
{
  ...
  g_free (self->vps_nals);
  g_free (self->sps_nals);
  g_free (self->pps_nals);
...
gst_h26x_base_parse_reset_frame (GstH26xBaseParse * self)
{
  ...
  if (klass->reset_frame)
    klass->reset_frame (self);
}
...
gst_h26x_base_parse_start (GstBaseParse * parse)
{
  ...
  gst_base_parse_set_min_frame_size (parse, self->min_nal_size + 1);
...
static void
gst_h26x_base_parse_format_from_caps (GstH26xBaseParse * self,
    GstCaps * caps, guint * format, GstH26xAlign * align)
{
  GstH26xBaseParseClass *klass = GST_H26X_BASE_PARSE_GET_CLASS (self);
  ...
  if (caps && gst_caps_get_size (caps) > 0) {
    ...
    if (format) {
      if ((str = gst_structure_get_string (s, "stream-format"))) {
        *format = klass->format_from_string (self, str);
      }
    }
...
static const gchar *
gst_h26x_base_parse_get_string (GstH26xBaseParse * self, gboolean format,
    gint code)
{
  GstH26xBaseParseClass *klass = GST_H26X_BASE_PARSE_GET_CLASS (self);

  if (format) {
    return klass->format_to_string (self, code);
  } else {
    switch (code) {
...
static void
gst_h26x_base_parse_negotiate (GstH26xBaseParse * self, guint in_format,
    GstCaps * in_caps)
{
  GstCaps *caps;
  guint format = self->format;
  GstH26xAlign align = self->align;
  ...
  /* default */
  if (format == GST_H26X_BASE_PARSE_FORMAT_NONE)
    ...
    GstBaseParseFrame * frame, gint * skipsize)
{
  ...
  gint framesize;
  GstFlowReturn ret;
  GstH26xHandleFrameReturn handle_ret;
  ...
  if (G_UNLIKELY (size < self->min_nal_size)) {
  ...
  if (self->current_off == -1) {
    ...
    switch (handle_ret) {
      case GST_H26X_HANDLE_FRAME_DROP:
        frame->flags |= GST_BASE_PARSE_FRAME_FLAG_DROP;
        gst_buffer_unmap (buffer, &map);
        ret = gst_base_parse_finish_frame (parse, frame, framesize);
        goto drop;
      case GST_H26X_HANDLE_FRAME_SKIP:
        goto skip;
      case GST_H26X_HANDLE_FRAME_INVALID_STREAM:
        goto invalid_stream;
      default:
        break;
    }
  ...
  switch (handle_ret) {
    ...
    case GST_H26X_HANDLE_FRAME_INVALID_STREAM:
      goto invalid_stream;
    case GST_H26X_HANDLE_FRAME_MORE:
      goto more;
    default:
      break;
  }
  ...
  gst_h26x_base_parse_parse_frame (GST_H26X_BASE_PARSE (parse), frame);
...
static gboolean
gst_h26x_base_parse_handle_parameter_set_nals (GstH26xBaseParse * self,
    GstBuffer * buffer, GstBaseParseFrame * frame)
{
  ...
  if (self->align == GST_H26X_BASE_PARSE_ALIGN_NAL) {
    ...
    GST_DEBUG_OBJECT (self,
        "- sending %sSPS/PPS", self->max_vps_count ? "VPS/" : "");
    for (i = 0; i < self->max_vps_count; i++) {
    ...
    GST_DEBUG_OBJECT (self,
        "- inserting %sSPS/PPS", self->max_vps_count ? "VPS/" : "");
    for (i = 0; i < self->max_vps_count; i++) {
      if ((codec_nal = self->vps_nals[i])) {
    ...
    if (G_UNLIKELY (!ok)) {
      GST_ERROR_OBJECT (self, "failed to insert SPS/PPS");
    }
  }
    ...
    GstBaseParseFrame * frame)
{
  ...
  if (!self->sent_codec_tag) {
    ...
    /* codec tag */
    caps = gst_pad_get_current_caps (GST_BASE_PARSE_SRC_PAD (self));
    if (caps == NULL) {
  ...
  if (self->interval > 0 || self->push_codec) {
    ...
    if (self->idr_pos >= 0) {
      ...
      if (GST_TIME_AS_SECONDS (diff) >= self->interval ||
        ...
        if (gst_h26x_base_parse_handle_parameter_set_nals (self, buffer, frame)) {
    ...
    if (self->idr_pos >= 0) {
      ...
      gst_h26x_base_parse_handle_parameter_set_nals (self, buffer, frame);
   ...
   * For nal aligned multiresoluton streams, passthrough mode make parse
  ...
  if (parse->can_passthrough) {
    if (parse->keyframe && parse->have_sps && parse->have_pps) {
...
gst_h26x_base_parse_set_caps (GstBaseParse * parse, GstCaps * caps)
{
  ...
  old_caps = gst_pad_get_current_caps (GST_BASE_PARSE_SINK_PAD (self));
  if (old_caps) {
  ...
  } else if (IS_PACKETIZED_FORMAT (format)) {
...
packetized_caps_codec_data_wrong_type:
  {
    GST_WARNING_OBJECT (self, "H.264 AVC caps, codec_data field not a buffer");
    goto refuse_caps;
  }
packetized_caps_codec_data_missing:
  {
    GST_WARNING_OBJECT (self, "H.264 AVC caps, but no codec_data");
    goto refuse_caps;
  }
refuse_caps:
  {
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  self = GST_H26X_BASE_PARSE (object);
    ...
    GstBaseParseFrame * frame)
{
  ...
  if (self->do_ts && klass->get_timestamp)
...
GstBuffer *
gst_h26x_base_parse_wrap_nal (GstH26xBaseParse * self,
...
gst_h26x_base_parse_update_src_caps (GstH26xBaseParse * self, GstCaps * caps)
{
  ...
  GstStructure *s = NULL;
  GstH26xBaseParseClass *klass = GST_H26X_BASE_PARSE_GET_CLASS (self);
  gboolean has_sps = klass->has_last_sps (self);
  ...
  if (IS_PACKETIZED_FORMAT (self->format)
  ...
  if (caps) {
    ...
    if (has_sps) {
      GstH26xProfileTierLevel str = { 0 };
      klass->fill_profile_tier_level (self, &str);

      if (str.profile != NULL)
        gst_caps_set_simple (caps, "profile", G_TYPE_STRING, str.profile, NULL);

      if (str.tier != NULL) {
        gst_caps_set_simple (caps, "tier", G_TYPE_STRING, str.tier, NULL);
      }

      if (str.level != NULL)
        gst_caps_set_simple (caps, "level", G_TYPE_STRING, str.level, NULL);
