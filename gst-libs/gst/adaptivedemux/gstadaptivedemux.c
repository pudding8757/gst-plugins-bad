    ...
    GstAdaptiveDemuxStream * stream)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_CAPS:
    {
      GstAdaptiveDemuxOutputSlot *slot, *to_free = NULL;
      GstMessage *msg = NULL;
      ...
      if (slot->exposed) {
        ...
        if (g_strcmp0 (gst_structure_get_name (str),
          ...
          to_free = slot;
          gst_adaptive_demux_slot_unlink (to_free);

          slot = gst_adaptive_demux_create_slot (demux,
              gst_stream_get_stream_type (stream->stream_object));
          demux->priv->slots = g_list_append (demux->priv->slots, slot);
          demux->priv->slots = g_list_remove (demux->priv->slots, to_free);
      ...
      if (to_free) {
        gst_pad_push_event (to_free->src_pad, gst_event_new_eos ());
        GST_MANIFEST_LOCK (demux);
        gst_adaptive_demux_slot_free (to_free);
        GST_MANIFEST_UNLOCK (demux);
      }
